...
export default function MatchDetailsScreen() {
  ...
  const handleRecording = async () => {
    if (isRecording) {
      console.log('Stop Recording triggered from handleRecording');
      await stopRecording();
    } else if (!isRecordingInProgress){
    
        setIsRecordingInProgress(true);
        const videoUri2 = await startRecording();
        if (videoUri2){
  ...
  const handleRecording2 = async () => {
    if (isRecording) {
      console.log('Stop Recording triggered from handleRecording2');
      await stopRecording();
    } else if (!isRecordingInProgress){
  ...
  const startRecording = async () => {
    ...
    if (cameraRef.current && cameraReady && !isRecording) {
      try {
        ...
        const videoUri = await cameraRef.current.recordAsync({ quality: '720p', mute: false ,  orientation: 'portrait', deviceOrientation: 'portrait', videoBitrate: 4000000, fps: 60, codec: 'h264' }); //10mb , maxFileSize: 10000000, maxDuration: 10
        if (videoUri?.uri){
            console.log('Video recorded to:', videoUri.uri);
            if (whichButtonPressed === 'four' || whichButtonPressed === 'six' || whichButtonPressed === 'wicket') {
              setIsRecording(false);
              const asset = await MediaLibrary.createAssetAsync(videoUri.uri);
              console.log('Asset created:', asset.uri);
            } else {
              console.log('No asset created');
              setIsRecording(false);
            }
            setIsRecording(false);
            setIsRecordingInProgress(false);
  ...
  //         <TouchableOpacity
  //           style={styles.button}
  //           onPress={()=> setHasAllPermissions(false)}>
  ...
  const handleSelectBowler  = async (player: Player) => {
  ...
  const waitForRecordingToStop = async () => {
    console.log('Waiting for recording to stop: ',isRecording);
    setIsRecording(false);
    console.log('Waiting for recording to stop: ',isRecording);
    let attempts =0;
    while (isRecording) {
      await new Promise((resolve) => setTimeout(resolve, 5000)); // Check every 500ms
      console.log("Checking isRecording ",attempts," times: ", isRecording);
      attempts++;
      if (!isRecording) break;
    }
    console.log("Recording stopped, proceeding...");
  };

  const callbattingAddOneRuns = async () =>{
    setWhichButtonPressed('one');
    await stopRecording();
