...
interface VideoProcessingResponse {
  trajectory_points: any[]; // Adjust type as needed
  prediction_result: string;
  ball_hits_batsman: boolean;
}
...
export default function MatchDetailsScreen() {
  ...
  const [isDRSModalVisible, setDRSModalVisible] = useState(false);
  const [result, setResult] = useState<VideoProcessingResponse | null>(null);
  const [videoUrl, setVideoUrl] = useState<string | null>(null);
  const [DRSloading, setDRSLoading] = useState<boolean>(false);
  const SERVER_URL = "http://192.168.101.138:5000";
  // const SERVER_URL = "http://172.17.8.162:5000";
  ...
  const handleDRSButtonPress = async () => {
    ...
    setHighlightContent(true);
    setIsRecording(false);
    setDRSModalVisible(true); // Open the DRS modal
  }

  const uploadDRSVid=async()=>{
    try{
      setDRSLoading(true);
      ...
      formData.append('video', {
        uri: videoUri,
        name: res.assets[0].name || 'video.mp4',
        type: res.assets[0].mimeType || 'video/mp4'
        } as any);
      ...
      const responseData = await response.json();
        ...
        setResult({
            prediction_result: responseData.prediction_result || 'No result available',
            trajectory_points: responseData.trajectory_points || [],
            ball_hits_batsman: responseData.ball_hits_batsman || false
        });
      ...
      Alert.alert('Error', 'Failed to process video. Check console for details.');
    }finally{
      setDRSLoading(false);
    }
  }
  ...
  const pressedDRS = async (url: string) =>{
    ...
    console.log("In the pressed DRS func, url is ",url);
  ...
  const startRecording = async () => {
    ...
    if (highlightContentRef.current) {
      try{
        ...
        //check if drs button is pressed or not
        if(drsRef.current){
          console.log("in desRef condition")
          // const result = await pressedDRS(urls[0]);
          const result = await pressedDRS(Hvideo.uri);
        }
    ...
    <View style={styles.container}>
      {loading ? (
        ...
        <CameraView mode='video'
          ...
          <Modal
            visible={isDRSModalVisible}
            transparent={true}
            animationType="slide"
            onRequestClose={() => setDRSModalVisible(false)}
          >
            <View style={styles.DRSmodalContainer}>
              <View style={styles.DRSmodalContent}>
                <Text style={styles.DRSmodalText}>DRS Review</Text>
                {/* Add your DRS content here */}

          {/* Video Upload and Prediction Section */}
            <View style={{ padding: 20, alignItems: 'center' }}>
                  {/* <Button title="Upload Video" onPress={uploadDRSVid} /> */}
                  ...
                  {result && (
                    <View style={{ marginTop: 20 }}>
                      <Text style={{ fontSize: 16, fontWeight: 'bold', textAlign: 'center' }}>
                        Prediction Result: {result.prediction_result}
                      </Text>
                    </View>
                  )}
                ...
                <TouchableOpacity
                  onPress={() => setDRSModalVisible(false)}
                  style={styles.DRSmodalButton}
                >
                  <Text style={styles.DRSbuttonText}>Close</Text>
                </TouchableOpacity>
              </View>
            </View>
          </Modal>
